...
import signal
import sqlite3
from funciones_generales import conectar_bd, nueva_pos_taxi
import numpy as np
    ...
    while True:
        ...
        for message in consumer:
            ...
            if (buscarCliente(conexion, cliente_id) == False):
                    ...
                    agregarCliente(conexion, cliente_id, destino, "EN ESPERA", coordX, coordY)
...
def procesar_coordenadas_taxi(taxi_id, coordX_taxi, coordY_taxi, broker):
    ...
    try:
        ...
        for cliente in clientes_a_mostrar_global[:]:  # Usar una copia de la lista
            ...
            # Si no se ha recogido a un cliente, agregar taxi en movimiento
            taxis_en_tablero.append((taxi_id, coordX_taxi, coordY_taxi, 1, None))
        ...
        # Si el taxi no ha recogido a ningún cliente, asegurarse de que siga en movimiento
        taxis_en_tablero.append((taxi_id, coordX_taxi, coordY_taxi, 1, None))  # Siempre agregar taxi
...
def actualizar_tablero(ax, destinos, clientes, taxis):
    ...
    # Agregar taxis
    for taxi_id, coordX_taxi, coordY_taxi, estado, cliente_en_taxi in taxis:
        color_taxi = "green" if estado == 1 else "red"  # Verde si está en movimiento, rojo si ha llegado
        ax.add_patch(plt.Rectangle((coordX_taxi - 1, coordY_taxi - 1), 1, 1, color=color_taxi))
        texto_taxi = f"{taxi_id}" if cliente_en_taxi is None else f"{taxi_id}-{cliente_en_taxi}"
        ax.text(coordX_taxi - 0.5, coordY_taxi - 0.5, texto_taxi, fontsize=10, ha='center', va='center', color="black")
